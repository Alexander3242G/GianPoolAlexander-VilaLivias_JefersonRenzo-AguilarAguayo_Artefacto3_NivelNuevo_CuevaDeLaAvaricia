...
signal animation_finished_ball
...
func _on_animated_sprite_2d_frame_changed():
	...
	if _cannon_animation.frame == 3:
		...
		var ball = "scenes/game/levels/objects/damage_object/seaShell/sea_shell_ball.tscn"
		new_ball = load(ball).instantiate()
		# Acceder al AnimatedSprite2D de la bala
		var animated_sprite = new_ball.get_node("AnimatedSprite2D")
...
func _on_animated_sprite_2d_animation_finished():
	...
	var animated_sprite = new_ball.get_node("AnimatedSprite2D")
	if animated_sprite and (animated_sprite.get_animation() == 'destroyed' or animated_sprite.get_animation() == 'hit'):
